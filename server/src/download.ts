...
const loadAndSaveWeights = async(
    weightPath: string, weightsManifest: io.WeightsManifestConfig,
...
const mobileNetMultipliers = [0.50, 0.75, 1.0];
const mobileNetQuantBytes = [2, 4];

const getMobileNetCheckpoints = () => {
  const checkpoints: CheckpointConfig[] = [];

  mobileNetMultipliers.forEach(
      multiplier => {mobileNetOutputStrides.forEach(outputStride => {
...
const main =
  ...
  const saves = checkpoints.map(
      checkpoint => loadAndSaveModel(checkpoint, modelSaveFolder));
